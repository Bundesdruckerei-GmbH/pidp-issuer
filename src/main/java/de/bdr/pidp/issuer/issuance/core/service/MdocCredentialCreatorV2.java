...
@Validated
public class MdocCredentialCreatorV2 implements CredentialCreator {
    ...
    public MdocCredentialCreatorV2(@NotNull CredentialSigner signer, Period maxValidity, String docType) {
        this.signer = signer;
    ...
    public String create(PIDIdentityData data, JWK holderBindingKey, StatusReference status, Validity validity) {
        ...
        var maxExpiration = LocalDate.ofInstant(validFrom, localZoneId).plus(maxValidity);
        var expiration = getExpirationDate(data, maxExpiration);

        var pidNamespace = new MdocPIDNamespaceBuilderV2(EU_NAMESPACE)
            .personalData(data)
            .expiration(expiration)
            .issuingCountry(ISO3166_1Alpha2CountryCode.DE)
            .build();
        var dePidNamespace = new MdocPIDNamespaceBuilderV2(DE_NAMESPACE)
            .personalDataDe(data)
            .build();

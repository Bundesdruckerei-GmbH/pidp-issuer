...
class MdocCredentialCreatorV1Test extends MdocCredentialCreatorTestBase {
    ...
    private static MdocCredentialCreatorV1 subject;
    ...
    static void setUp() throws UnrecoverableKeyException, CertificateException, IOException, KeyStoreException, NoSuchAlgorithmException {
        var mdocMetadata = (CredentialConfiguration.Mdoc) ISSUANCE_METADATA.credentialConfigurationsSupported().get(CredentialConfigurationID.MSO_MDOC_V1);
        var signer = new PKCS12CredentialSigner(ISSUANCE_CONFIG);
        subject = new MdocCredentialCreatorV1(signer, mdocMetadata.doctype());
    ...
    void test001() {
        ...
        // When
        var mdoc = subject.create(data, bindingKey, statusRef, validityV1);
    ...
    class MdocNamespacesTest {
        ...
        @DisplayName("Verify mdoc V1 contained all eu claims")
        @Test
        void test001() {
            // Given
            var statusRef = getStatusReference();
            ...
            assertSoftly(soft -> {
                soft.assertThat(pidValues)
                    .extractingByKey("issuance_date")
                    .isNotNull()
                    .extracting(i -> Instant.parse((String) i))
                    .isEqualTo(TODAY);
                soft.assertThat(pidValues)
                    .extractingByKey("expiry_date")
                    .isNotNull()
                    .extracting(i -> Instant.parse((String) i))
                    .isEqualTo(TODAY.plus(LIFETIME));
                soft.assertThat(pidValues).hasSize(22);
                ...
                soft.assertThat(pidValues).extractingByKey("birth_place").asInstanceOf(map(String.class, String.class))
                ...
                soft.assertThat(pidValues).extractingByKey("family_name_birth").isEqualTo("GABLER");
                soft.assertThat(pidValues).extractingByKey("source_document_type").isEqualTo("ID");
            });
        }

        @DisplayName("Verify mdoc V1 built eu pid namespace correct with minimal data")
        @Test
        void test002() {
            // Given
            var statusRef = getStatusReference();
            ...
            assertSoftly(soft -> {
                soft.assertThat(pidValues).hasSize(15);
                soft.assertThat(pidValues).extractingByKey("family_name").isEqualTo("MUSTERMANN");
                soft.assertThat(pidValues).extractingByKey("given_name").isEqualTo("ERIKA");
                soft.assertThat(pidValues).extractingByKey("birth_date").isEqualTo("1964-08-12");
                soft.assertThat(pidValues).extractingByKey("nationality").asInstanceOf(list(String.class)).containsExactly("DE");
                soft.assertThat(pidValues).doesNotContainKey("birth_place");
                soft.assertThat(pidValues).doesNotContainKey("resident_address");
                soft.assertThat(pidValues).doesNotContainKey("resident_country");
                soft.assertThat(pidValues).doesNotContainKey("resident_state");
                soft.assertThat(pidValues).doesNotContainKey("resident_city");
                soft.assertThat(pidValues).doesNotContainKey("resident_postal_code");
                soft.assertThat(pidValues).doesNotContainKey("resident_street");
                soft.assertThat(pidValues).extractingByKey("issuing_authority").isEqualTo("DE");
                soft.assertThat(pidValues).extractingByKey("issuing_country").isEqualTo("DE");
                soft.assertThat(pidValues).extractingByKey("age_over_12").asInstanceOf(BOOLEAN).isTrue();
                soft.assertThat(pidValues).extractingByKey("age_over_14").asInstanceOf(BOOLEAN).isTrue();
                ...
                soft.assertThat(pidValues).extractingByKey("age_over_21").asInstanceOf(BOOLEAN).isTrue();
                soft.assertThat(pidValues).extractingByKey("age_over_65").asInstanceOf(BOOLEAN).isFalse();
                soft.assertThat(pidValues).doesNotContainKey("family_name_birth");
                soft.assertThat(pidValues).extractingByKey("source_document_type").isEqualTo("ID");
            });
        }

        @DisplayName("Verify issuance_date and expiry_date where encoded as 'tdate = #6.0(tstr)'")
        @Test
        void test003() {
            // Given
            var statusRef = getStatusReference();
            ...
            assertThat(nameSpaceDataElementStream)
                .anySatisfy(o -> {
                    assertThat(o.getElementIdentifier()).isEqualTo("issuance_date");
                    assertThat(o.getElementValue()).satisfies(MdocCredentialCreatorTestBase::assertTypeIsStandardDateTimeString);
                })
                .anySatisfy(o -> {
                    assertThat(o.getElementIdentifier()).isEqualTo("expiry_date");
                    assertThat(o.getElementValue()).satisfies(MdocCredentialCreatorTestBase::assertTypeIsStandardDateTimeString);
                });
        }
